// Not best practive -- Convert to .addEventListener's
...
function userDifSelection() {
    const localStorageValue: number = JSON.parse(localStorage.getItem('difficultyKey') || '{}');
    if (localStorage.getItem('difficultyKey') !== null) {
        (<HTMLInputElement>document.getElementById('difficulty-select')).value = String(localStorageValue);
        return localStorageValue;
    }
    else {
        let dif: string = (<HTMLInputElement>document.getElementById('difficulty-select')).value
...
for (let i: number = 0; i < gameSettingObj.gameSize; i++) {
    ...
    for (let j: number = 0; j < gameSettingObj.gameSize; j++) {
        ...
        cell.addEventListener('contextmenu', function(ev: any) {
            ev.preventDefault();
            rightClick(this.id);
            return false;
        }, false);
    ...
    (<HTMLInputElement>document.getElementById("canvas")).appendChild(row)
}
...
let cellGameStateObj: {[key: string]: Cell} = {};
...
for (let cell of gameStateCoordinates) {
    const objName: string = `cell_${cell[0]}_${cell[1]}`;
...
function neighborsArray(xy: string[]) {
    // Define empty array to receive coordinates
    let neighborArray: number[][] = [];
    // Get x, y coordinates as numbers
    const cellX: number = parseInt(xy[0]);
    const cellY: number = parseInt(xy[1]);
...
function proxIndicators() {
    for (let obj in cellGameStateObj) {
        if (cellGameStateObj[obj].isVisible === true && cellGameStateObj[obj].proxNum > 0 && cellGameStateObj[obj].bombInd === false) {
            ...
            // Insert proxNum
            (<HTMLInputElement>document.getElementById(tempCellId)).innerText = String(cellGameStateObj[obj].proxNum);
            // Styling
            (<HTMLInputElement>document.getElementById(tempCellId)).style.fontWeight = 'bold';
            (<HTMLInputElement>document.getElementById(tempCellId)).style.fontSize = '25px';
            (<HTMLInputElement>document.getElementById(tempCellId)).style.lineHeight = '100%';
            // Define proxNum-sepcific styling
            if (cellGameStateObj[obj].proxNum === 1) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'blue'
            else if (cellGameStateObj[obj].proxNum === 2) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'green'
            else if (cellGameStateObj[obj].proxNum === 3) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'red'
            else if (cellGameStateObj[obj].proxNum === 4) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'purple'
            else if (cellGameStateObj[obj].proxNum === 5) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'maroon'
            else if (cellGameStateObj[obj].proxNum === 6) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'turquoise'
            else if (cellGameStateObj[obj].proxNum === 7) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'black'
            else if (cellGameStateObj[obj].proxNum === 8) (<HTMLInputElement>document.getElementById(tempCellId)).style.color = 'grey'
        }
    }
}
...
function leftClick(id: string) {
    ...
    if (bombChecker(clickedID)) {
        ...
        (<HTMLInputElement>document.getElementById(`${clickedID[0]} ${clickedID[1]}`)).innerHTML = '💣';
        ...
        (<HTMLInputElement>document.getElementById(`${clickedID[0]} ${clickedID[1]}`)).style.backgroundColor = 'red';
        bombShowAll(numLives.lives);
        remainingLives();
    }
    // Actions
    else if (!bombChecker(clickedID)) {
        (<HTMLInputElement>document.getElementById(id)).style.backgroundColor = 'rgb(245, 245, 245)';
...
function visBombArray(newBombCells: string[]) {
    newBombCells.forEach(ab => {
        (<HTMLInputElement>document.getElementById(`${ab}`)).style.backgroundColor = 'rgb(245, 245, 245)';
        (<HTMLInputElement>document.getElementById(`${ab}`)).innerHTML = '💣';
        (<HTMLInputElement>document.getElementById(`${ab}`)).style.fontSize = '20px';
        (<HTMLInputElement>document.getElementById(`${ab}`)).style.lineHeight = '120%';
        cellGameStateObj[`cell_${ab.split(' ')[0]}_${ab.split(' ')[1]}`].isVisible = true;
    });
}
...
function flagCountUpdate() {
    (<HTMLInputElement>document.getElementById("flagCounter")).innerHTML = String('🚩 ' + numFlags.flags);
}
...
function remainingLives() {
    ...
    (<HTMLInputElement>document.getElementById("livesCounter")).innerHTML = lifesString;
}
...
function scoreCounter() {
    ...
    (<HTMLInputElement>document.getElementById("score")).innerHTML = htmlScore;
}
...
function playerWonTrue() {
    ...
    (<HTMLInputElement>document.getElementById("livesCounter")).innerHTML = lifesString;
    freezeGame();
    alert(`You Won!!! ${(<HTMLInputElement>document.getElementById("score")).innerHTML}`);
}
...
function playerLostTrue() {
    alert(`Game Over - ${(<HTMLInputElement>document.getElementById("score")).innerHTML}`);
}
...
function timer() {
    ...
    let timer = setInterval(function(){
        if (playerWonChecker() === false && playerLostChecker(numLives.lives) === false) {
            (<HTMLInputElement>document.getElementById('timer')).innerHTML = `${String(timeFormat(sec))}`;
            sec++;
        }
    }, 1000);
    return timer;
